...
import { GameCharacter } from "@roll4init/common";
import { CharacterNodeHelper } from "../../helpers/character";

// load character.graphql
let GameCharacterSchema: string;
readFile(
    join(__dirname, "..", "schemas", "game-character.graphql"),
    "utf-8"
).then(data => (GameCharacterSchema = data));

let GameCharacterResolvers = {
    GameCharacter: {
        StrengthMod: (character: GameCharacter) =>
